...
int main() {
    ...
    std::printf("MooErrorDetail (observed-only / observation-corrected entries):\n");
    check_eq(MooErrorDetail("Moo_DownloadFile|Err_4"), "File download cancelled by user.", "DownloadFile Err_4 (observed return string)");
    check_eq(MooErrorDetail("Moo_ZipCompress|Err_4"), "File already exists in Zip archive.", "ZipCompress Err_4 (observed return string)");
    check_eq(MooErrorDetail("Moo_ZipCompress|Err_6"), "Output folder doesn't exist.", "ZipCompress Err_6 (observed return string)");
    check_eq(MooErrorDetail("Moo_FolderList|Err_4"), "Input folder doesn't exist.", "FolderList Err_4 (observed return string)");
    check_eq(MooErrorDetail("Moo_FolderList|Err_5"), "Unknown error.", "FolderList Err_5 (observed return string)");
    check_eq(MooErrorDetail("Moo_FileMove|Err_7"), "Error deleting destination file.", "FileMove Err_7 (observed return string)");
    check_eq(MooErrorDetail("Moo_FileInfo|Err_7"), "Error setting file info.", "FileInfo Err_7 (observed return string)");
    check_eq(MooErrorDetail("Moo_ZipExtract|Err_12"), "Password Required to Extract File.", "ZipExtract Err_12 (observed return string)");

    std::printf("MooErrorDetail (observed verbatim quirks):\n");
    ...
    check_eq(MooErrorDetail("Moo_FTPDelete|Err_1"), "", "FTPDelete has no ErrorDetail description");
