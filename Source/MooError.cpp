...
//
// エラー表は MooPlug 0.4.9 実バイナリ（mooplug_049/MooPlug.fmx）の文字列プール
// から復元した。プール内では Moo_ErrorDetail の比較チェーンがコード→説明文の
// 初出順に並んでおり（MSVC の文字列プールは初出順・同一リテラルは共有）、
// その並びと Error Descriptions ページ・各関数詳細ページを突き合わせて確定した。
// ドキュメントとバイナリが食い違う箇所はバイナリを採用し、コメントで注記する。
...
const char* const kMooVersionString = "MooPlug 0.4.9";
...
const std::unordered_map<std::string, std::string>& ErrorTable() {
    static const std::unordered_map<std::string, std::string> table = {
        ...
        // 実機の DialogFile キャンセル時の戻りは "MooDialogFile|Err_2.<code>"
        //（Moo_ の _ 抜け + 数値サフィックス付き）だが、ErrorDetail の表は正規形を引く
        ...
        // ドキュメントは "User cancelled download." だがバイナリはこちら
        ...
        // Moo_DownloadText|Err_6 はバイナリの戻り値側にだけ存在し、説明文は特定不能（未登録）
        ...
        // プール初出順から推定（bOverwrite で既存先を消す処理の失敗）。
        // FileMove|Err_7 と違い "destination" 無し。TODO-compat: 実機照合
        ...
        // ドキュメントの Moo_FileWrite|Err_6 (Error removing file.) は 0.4.9 バイナリに
        // 存在しない（bOverwrite 引数ごと消えた 0.4.7 時代の名残）ので登録しない
        ...
        // 実機の戻りは "Moo_FolderCopy|Err_6.<n>  (<hex>)" 形式だが表は正規形
        ...
        // Err_4/Err_5 はドキュメントに無い（バイナリのプール並びから復元）
        ...
        // Err_8/9 のドキュメント文言は FTPUpload からのコピペ。バイナリのプール並びでは
        ...
        // Moo_FTPDelete|Err_1〜8 は実機の ErrorDetail チェーンに無い（未登録のまま）
        ...
        // Moo_ProcessKill|Err_3 はバイナリの戻り値側にだけ存在し、説明文は特定不能（未登録）
        ...
        // 末尾ピリオド無しはバイナリ原文ママ（ZipExtract|Err_5 とは別リテラル）
