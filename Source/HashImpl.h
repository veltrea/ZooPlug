...
// 外部ライブラリに依存しない自己完結実装（本家 MooPlug は hashlib++ を同梱していた。
// ZooPlug は OpenSSL 等を避けるため自前実装。NIST テストベクタで検証 — tests/test_hash.cpp）。
// FileMaker SDK (FMWrapper) に一切依存しない。
//
// 出力は小文字 16 進（0.4.9 実バイナリの "0123456789abcdef" リテラルと整合）。
